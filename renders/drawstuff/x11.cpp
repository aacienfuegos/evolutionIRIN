...
extern "C" void dsError (char *msg, ...)
{
  va_list ap;
  va_start (ap,msg);
  printMessage ((char*) "Error",msg,ap);
  exit (1);
}
...
extern "C" void dsDebug (char *msg, ...)
{
  va_list ap;
  va_start (ap,msg);
  printMessage ((char*) "INTERNAL ERROR",msg,ap);
...
static void createMainWindow (int _width, int _height)
{
  ...
  if (!display) dsError ((char*) "can not open X11 display");
  ...
  if (!visual) dsError ((char *) "no good X11 visual found for OpenGL");
  ...
  if (width < 1 || height < 1) dsDebug (0,(char*) "bad window width or height");
  ...
  if (!glx_context) dsError ((char*) "can't make an OpenGL context");
  ...
  if (XSetWMProtocols (display,win,&wm_delete_window_atom,1)==0)
    dsError ((char*) "XSetWMProtocols() call failed");
...
static void captureFrame (int num)
{
  ...
  FILE *f = fopen (s,"wb");
  if (!f) dsError ( (char*) "can't open \"%s\" for writing",s);
  fprintf (f,"P6\n%d %d\n255\n",width,height);
