...
const std::string DEFAULT_PATH_TO_TEXTURES = "..\\textures\\";
#else
//#define DEFAULT_PATH_TO_TEXTURES "../textures/"
const std::string DEFAULT_PATH_TO_TEXTURES = "../textures/";
#endif
...
static void drawCylinder (float l, float r, float zoffset)
{
  ...
  const int n = 40;	// number of sides to the cylinder (divisible by 4)
...
{
  if (current_state < 1) dsDebug ( (char*) "internal error");
  current_state = 2;
...
//   printf ("%f %f %f\n%f %f %f\n", view2_xyz[0],view2_xyz[1],view2_xyz[2],
// 	     view2_hpr[0],view2_hpr[1],view2_hpr[2]);
...
extern "C" void dsSimulationLoop (int argc, char **argv,
				  ...
				  dsFunctions *fn)
{
  if (current_state != 0) dsError ((char*) "dsSimulationLoop() called more than once");
  current_state = 1;
  ...
  if (fn->version > DS_VERSION)
    dsDebug ((char*) "bad version number in dsFunctions structure");
...
{
  if (current_state < 1) dsError ( (char*) "dsSetViewpoint() called before simulation started");
  if (xyz) {
    view_xyz[0] = xyz[0];
    view_xyz[1] = xyz[1];
    view_xyz[2] = xyz[2];
...
{
  if (current_state < 1) dsError ( (char*) "dsGetViewpoint() called before simulation started");
  if (xyz) {
    xyz[0] = view_xyz[0];
    xyz[1] = view_xyz[1];
    xyz[2] = view_xyz[2];
...
{
  if (current_state != 2) dsError ( (char*)"drawing function called outside simulation loop");
  tnum = texture_number;
}
