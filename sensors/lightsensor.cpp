...
double CLightSensor::m_fLightSensorDir[NUM_LIGHT_SENSORS] 	= {0.2967, 0.8727, 1.5708, 2.6180, 3.6652, 4.7124, 5.4105, 5.9865};
unsigned int CLightSensor::SENSOR_NUMBER = NUM_LIGHT_SENSORS;

/******************************************************************************/
/******************************************************************************/

CLightSensor::CLightSensor(const char* pch_name, double f_rangeLightSensor) :
...
double* CLightSensor::ComputeSensorReadings(CEpuck* pc_epuck, CSimulator* pc_simulator){
  ...
  CArena* pcArena = pc_simulator->GetArena();
	...
	double fDistance;
	if ( pcArena->GetNearestLight(vPosition, m_fRangeLightSensor, &vLightPosition, &fDistance))
	{
...
unsigned int CLightSensor::GetType(){
	return SENSOR_LIGHT;
}
...
double* CLightSensor::GetSensorReading( CEpuck *p_pcEpuck){
...
double CLightSensor::GetMaxRange ( void ) 
{
...
const double* CLightSensor::GetSensorDirections()
{
...
/******************************************************************************/
/******************************************************************************/
void CLightSensor::SwitchNearestLight ( int n_value )
{
	dVector2 vPosition;
